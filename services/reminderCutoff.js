...
async function runCutoffReminders({ dryRun = false, skipWindow = false } = {}) {
    if (process.env.ENGAGE_ENABLED !== 'true' && !dryRun) {
        console.log('[cutoff-reminder] ENGAGE_ENABLED=false — skipping');
    }
    ...
    for (const t of tournamentsRes.rows) {
        ...
        for (const row of missingRes.rows) {
            ...
            const cutoffMinutes = skipWindow ? null : await getTournamentCutoffMinutes(t.tournament_id);
            const cutoffMs = skipWindow ? null : new Date(t.first_match_start).getTime() - cutoffMinutes * 60 * 1000;
            const minutesLeft = cutoffMs ? Math.round((cutoffMs - Date.now()) / 60000) : null;
            ...
            if (dryRun) {
                preview.push({
                    ...
                    match: `${t.first_home} vs ${t.first_away}`,
                });
                notified++;
                continue;
            }
    ...
    console.log(`[cutoff-reminder] tournaments_in_window=${tournamentsInWindow} standalone=${standaloneRes.rows.length} notified=${notified} skipped=${skipped} dryRun=${dryRun} skipWindow=${skipWindow}`);
    return {
